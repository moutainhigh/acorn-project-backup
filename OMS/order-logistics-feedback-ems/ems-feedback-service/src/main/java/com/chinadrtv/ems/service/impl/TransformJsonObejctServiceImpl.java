...
import com.chinadrtv.ems.bean.Listexpressmail;
import com.chinadrtv.ems.bean.Listexpressmails;
...
public class TransformJsonObejctServiceImpl implements TransformJsonObejctService  {
    private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(TransformJsonObejctServiceImpl.class);
    @Autowired
        ...
        try{
            ...
            if(temp != null){
                ...
                for(Listexpressmail lp : temp.getListexpressmail()){
                    ...
                    String companyId = shipmentHeaderDao.findByMailId(lp.getMailnum());
                    /*List<ShipmentHeader> shipmentHeaderList=shipmentHeaderDao.findByMailId(lp.getMailnum());
                    if(shipmentHeaderList!=null&&shipmentHeaderList.size()>0)
                    {
                        companyId=shipmentHeaderList.get(0).getEntityId();
                    }*/
            ...
            logger.error("transform json error:",e);
            logger.error("transfrom data:"+strJson);
            throw e;
        }
    }
