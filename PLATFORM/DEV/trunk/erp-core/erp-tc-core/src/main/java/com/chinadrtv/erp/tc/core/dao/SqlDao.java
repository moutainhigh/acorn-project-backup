 ...
 *
 */
@Component("com.chinadrtv.erp.tc.core.dao.SqlDao")
public class SqlDao{
	...
	* @throws
    ...
    public String exeFuncProcedure(final String spName, final List<Object> inParams)
            throws SQLException {
        ...
        result = jdbcTemplate.execute(new CallableStatementCreator() {
            public CallableStatement createCallableStatement(Connection con) throws SQLException {
                ...
                //输入参数
                for (int i = 2; i < inParams.size() + 2; i++) {
                    if (null == inParams.get(i - 2)) {
                        cs.setObject(i, "");
                    } else {
                        cs.setObject(i, inParams.get(i - 2));
                    }
                }
                return cs;
                }
