...
function singleStepConference(otherDn) {
    if(!checkSoftPhoneApplet()){
        return false;
    }
    SoftPhone.singleStepConference(1,otherDn,"{params:{UserData:{command:'3'}}}");
...
function muteOff() {
    ...
    console.info("muteOff");
    SoftPhone.muteOff(1,1);
...
function getDnStatus(dn) {
    console.info("getDnStatus_1:"+checkSoftPhoneApplet());
    if(!checkSoftPhoneApplet()){
        console.info("getDnStatus_2:"+checkSoftPhoneApplet());
        return false;
    }
    var result = SoftPhone.getDnStatus(dn);
    console.info("getDnStatus_3:"+result);
    return result == 0;
...
function onTalking(ani,dnsi,tollFreeNum,isOutBoound,company,province){
    $('#callContext').html('<span class="insure">'+ phone.insure+'</span><span class="incoming incoming_ac fl"></span>' +
        ...
        '<span id="marquee" class="fl ml5 c_combo pl5"  style="height:25px;padding-right:18px">'+ani+'<span class="arrow"></span></span>' +
...
function w_outhone(){
    $('#w_outphone').window('open');
}


function w_outPhoneCall(){
    $('#w_outphone').window('close');
   var  phoneNo=  $('#d_outPhone').val();
    //呼出号码待定
	...
	$("#center .tabPage").each(function(){
		...
		$(this).click(function(){
			...
			if(pObj==null){
				...
				closeCurrentPanel();
				currentPanelObj = createPanel(id,'false',url,'static');

//                tabsMap.put(id,currentPanelObj);
 ...
 * @param panelId
 */
function createPanel(panelId,isNew,url,type){
    setPageH();
	...
	var p = new panelObject(panelId,true,null,type);
    tabsMap.put(panelId,p);
...
//    $("#tab_"+tabId+" .tab_content").scroll('bindE');
	$("#tabs-active li").removeClass('current');
	closeCurrentPanel();
	currentPanelObj = createPanel(tabId,isNew,url,'active');
    var tab = new panelObject(tabId,true,idx,'active');
...
function panelObject(id,status,idx,type){
	this.id="p_"+id;
    this.type = type;
    if(type=='static'){
	    this.tabId=id;
...
function msgSplash(title,msg,second)
{
    $.messager.show({
        title:title,
        msg:msg,
