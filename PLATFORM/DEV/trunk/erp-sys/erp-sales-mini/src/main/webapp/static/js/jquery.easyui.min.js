...
},beforePageText:"第",afterPageText:"页 共{pages}页",displayMsg:"当前显示 {from} - {to} 条记录 共 {total} 条记录",nav:{first:{iconCls:"pagination-first",handler:function(){
...
},message:"请输入一个有效的email地址"},url:{validator:function(_3e9){
...
},message:"请输入 {0}到{1}个字符"},remote:{validator:function(_3ec,_3ed){
var data={};
data[_3ed[1]]=_3ec;
...
$(_441).find(".calendar-title span").html(opts.year+" "+opts.months[opts.month-1]);
...
return tt.join($(_853).datetimebox("spinner").timespinner("options").separator);
};
function _855(){
...
_854.calendar.calendar({year:new Date().getFullYear(),month:new Date().getMonth()+1,current:new Date()});
    var _86b=$.data(_853,"datetimebox");
    if(_86b){
        $(_853).datetimebox("spinner").timespinner("setValue",_879(_870(_853)));
        $(_853).combo("setValue",opts.formatter(_870(_853))+' '+_879(_870(_853))).combo("setText",opts.formatter(_870(_853))+' '+_879(_870(_853)));
    }else{
        $(_853).combo("setValue",opts.formatter(new Date())).combo("setText",opts.formatter(new Date()));
    }
    $(_853).combo("hidePanel");
});
