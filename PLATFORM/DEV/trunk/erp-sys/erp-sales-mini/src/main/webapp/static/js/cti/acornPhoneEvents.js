...
function onTalking(ani,dnsi,tollFreeNum,isOutBoound,company,province){
    $('#callContext').html('<span class="insure">'+ phone.insure+'</span><span class="incoming incoming_ac fl"></span>' +
        ...
        if($('#callback').attr('value')==1)$('#onHook').replaceWith('<span class="onHook"></span>');
         //进线通话
         $('#callContext').animate({width:$('#callContext').find('.callin').width()+50,paddingLeft:'30px',paddingRight:0,marginRight:'55px'},0,function(){
             $(this).width('');
//                 .css('margin-right','55px');

             console.info("3333333:"+phone.customerId);
...
//            $('#num marquee').show();
    ...
    $('a.logout').unbind();
     $('table.keyboard a').unbind().bind('click',function(){ toggleSPE(this)});
    $('#transferPhoneBtn').unbind().bind('click',function(){
...
function onDialing(ani,dnsi,tollFreeNum,isOutBoound,company,province){
    ...
    $('#callContext').html('<span class="insure">'+ phone.insure+'</span><span class="incoming incoming_ac fl"></span>' +
        ...
        '<a id="onHook" title="挂机" class="onHook" href="javascript:void(0)"></a>' +
        '<span id="end" class="end"  title="结束会话"></span>');
    $('#callContext').animate({width:$('#callContext').find('.callin').width(),paddingLeft:'30px',paddingRight:0,marginRight:'55px'},'slow',function(){
        $(this).width('');
    });
    $('#onHook').one("click",function(){
    ...
    $('#hooksave').click(function(){
    	...
    	if($("#h_hookForm").form('validate')){
            ...
            $.ajax({
                ...
                data:{
                    isContact:isContact,
                    contactTime:$('#h_contactTime').datebox("getValue"),
                    remark:$("#h_remark").val(),
                    h_instId:w_instId,
                    ani:phone.ani,
                    dani:phone.dnsi,
                ...
                success: function(data){
                    ...
                    try{
                        ...
                        $('#hook').window('close');
                        $('#h_contactTime').datebox('setValue', '');
                        $('#h_contactTime').datebox('disable');
...
var changeIsContact=function(){
    ...
    $("#d_group").combobox({
//        url : '/common/getGroupByDept?deptId=' + param.id,
        valueField : 'id',
        textField : 'name',
        onSelect : function(param) {
...
var callLocalhost=function(node){
    if(phone.status == 7){
        outphone(phone.ani,'',phone.instId,phone.customerId,phone.customerType,1);
    }
}
   ...
   //$("#div_transferPhone").attr("class","c_combo retrieve");
    switch (istransferPhone){
        ...
        case 1:
            completeTransfer();
            istransferPhone=0;
            console.info('电话转接:completeTransfer');
            break
        case 2:    //取回
            releaseLine2();
            retrieve();
...
var initProvinceCity  =function(){
    ...
    if(!$.jStorage.get("city.json")){
        ...
        $.post("/static/plugin/city.json", function(data) {
            ...
            $('#citylist').combobox({data:$.jStorage.get("city.json")});
        });
