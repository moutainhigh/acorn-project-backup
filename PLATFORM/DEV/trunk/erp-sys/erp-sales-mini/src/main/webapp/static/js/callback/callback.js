...
function savaCallBack(){
    ...
    if (!$("#formAddCustomerInfo").form('validate')) {
        ...
        $.post("/common/callBack", {
            ...
            callbackDt: $("#callbackDt").datebox("getValue"),
            callbackRemark: $("#callbackRemark").val(),
            callbackPriority: $("#callbackPriority").combobox('getValue'),
            h_instId:parent.phone.instId,
            leadInterId:parent.phone.leadInterId,
            leadId:parent.phone.leadId,
            acdgroup: parent.phone.acdId,
            ani:parent.phone.ani,
            dnsi:parent.phone.dnsi,
            phn1:parent.phone1,
            phn2:parent.phone2,
            phn3:parent.phone3,
            time_length:parent.phone.seconds
    },function(data){
           if(data.result){
           //清空数据
               clearCallbackForm();
...
function callback_findcustomer_callback(customerId,name,sex){
    $("#name").val(name);
