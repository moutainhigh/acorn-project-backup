...
import com.chinadrtv.erp.knowledge.controller.BaseController;
...
public class Uploader {
		...
		try {
			...
			while (fii.hasNext()) {
				...
				if (!fis.isFormField()) {
					...
					String[] copyFile = PropertiesUtils.getValue("copyPic")
							.split(",");
					List<String> list = new ArrayList<String>();
					for (int i = 0; i < copyFile.length; i++) {
						list.add(copyFile[i] + this.url);
					}
					copyFile(PropertiesUtils.getValue("pic") + this.url, list);
	...
	private String getPhysicalPath(String path) {
		...
		// 上传目录
		String realPath = PropertiesUtils.getValue("pic");// this.request.getSession().getServletContext().getRealPath(servletPath);
