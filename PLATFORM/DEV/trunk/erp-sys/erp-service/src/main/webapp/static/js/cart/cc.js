    ...
    $('#creditCards-grid').datagrid({
        ...
        onLoadSuccess: function(data){
            ...
            for(var index in data.rows){
                ...
                if(row.bankName=='工行'){
                    $("#creditCardMobile").show();
                    getCreditCardsCellPhones()
                }else{
                    $("#creditCardMobile").hide();
                }
            }
...
//    var opts = $('#creditCards-grid').datagrid('options');
//    $(opts).attr("url", '/checkout/cc/default');
    ...
    $('#selectCards-grid').datagrid({
        ...
        columns:[[
            ...
            {field:'o',title:'操作',width:50,align:'center', formatter:function(value,row,index){
                return "<a class='link modify ' title='编辑' href='javascript:editCcCard("+ index +")'></a>";
            }}
        ]],
        remoteSort:false,
        singleSelect:false,
...
//    var opts = $('#selectCards-grid').datagrid('options');
//    $(opts).attr("url", '/checkout/cc/all');
...
//获取订购人手机列表
function getCreditCardsCellPhones() {
    $.get('/cart/getCellPhones/' + $("#payment_contactid").val(),
        function (data) {
            if (data != null && data.result.length != 0 && data.result != "[]") {
                ...
                for (var index in phones) {
                    ...
                    if (phone['prmphn'] == 'Y') {
                        $("#id_creditCardMobile_hidden").val(phone['phone'])
                        $("#id_creditCardMobile").val(phone['phoneStr'])
                    }
                }
                if(!$("#id_creditCardMobile_hidden").val() || !$("#id_creditCardMobile").val()){
                    var phone = phones[0];
                    $("#id_creditCardMobile_hidden").val(phone['phone'])
                    $("#id_creditCardMobile").val(phone['phoneStr'])
                }
