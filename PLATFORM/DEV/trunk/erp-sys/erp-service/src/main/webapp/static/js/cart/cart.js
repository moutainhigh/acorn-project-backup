$(function () {
    ...
    parentInfoSwitch(parentOrderId);

    $("#parentInfo_clear").click(function(){
        clearParentOrderId();
    });
});
    ...
    $('#cart_tabs').tabs({
        ...
        onSelect: function (title) {
            if (title == "购物车") {
                ...
                if(parentOrderId!=null)
                    $('#parentInfo_clear').css({display:'none'});
            }
        }
    });
...
function initOrderType() {
    ...
    if(parentOrderId!=null)
    {
        ...
        getOrderTypeUrl +="&parentId="+parentOrderId;
    }
...
function deleteShoppingCartProduct(productId) {
    $.get('/cart/deleteProduct/' + $("#shoppingcart_id").val() + '/' + productId+'/'+parentOrderId,
        function (data) {
            if (data.message != null) {
...
function addScmPromotion(pluid, prodname, slprc, giftSource) {
    ...
    $.post('/cart/addScmPromotion',
        {shoppingCartId:$("#shoppingcart_id").val() ,skuCode: pluid, productName: prodname, productType: prodtype, salePrice: slprc,giftSource: giftSource, parentOrderId:parentOrderId},
        function (data) {
            if (data.message != null) {
                window.parent.alertWin('系统提示', data.message);
                reloadAllShoppingCartProducts();
...
function addProduct(ncCode, ncFreeName,salePrice) {
    ...
    $.post('/cart/addShoppingCartProduct' ,
        {ncCode: ncCode, ncFreeName: ncFreeName,salePrice:salePrice, shoppingCartId:$("#shoppingcart_id").val(),parentOrderId:parentOrderId},
        function (data) {
            if (data.message != null) {
                window.parent.alertWin('系统提示', data.message);
                reloadAllShoppingCartProducts();
...
function getShoppingCartHtml(shoppingCartProductDto) {
    ...
    if(parentOrderId!=null)
    {
        for(var i=0;i<orderSaleTypeList.length;i++)
        {
            if(orderSaleTypeList[i].id.id==shoppingCartProductDto.isGift)
            {
                html += '<option value="'+orderSaleTypeList[i].id.id+'" selected="selected">'+orderSaleTypeList[i].dsc+'</option>';
            }
            else
            {
                html += '<option value="'+orderSaleTypeList[i].id.id+'">'+orderSaleTypeList[i].dsc+'</option>';
            }
        }
        html+='</td>';
    }
...
function updateShoppingCartProduct(shoppingCartProductId, property) {
    ...
    $.ajax({url: '/cart/updateShoppingCartProduct',
        data: "id=" + shoppingCartProductId + "&" + property+"&parentOrderId="+parentOrderId,
    ...
    $.get('/cart/getShoppingCartProduct/'+ shoppingCartProductId,
        function (data) {
            if (data != null && data.length != 0 && data != "[]") {
                ...
                var saleType=$("#scp_" + shoppingcartProduct.id).find("select[name='select4']");
                if(saleType!=null&&saleType.length==1)
                {
                    $(saleType[0]).val(shoppingcartProduct.isGift);
                }
...
//潜客升级
var potentialContactTabId ='';
function updateCustomer() {
    var potentialContactCode = $("#customerId").val();
    potentialContactTabId =potentialContactCode;
...
function sendMessage() {
    ...
    for (var i = 0; i < smsTemplateList.length; i++) {
        ...
        if (smsTemplate.name == $("#id_message_select").val()) {
            ...
            sendCartSms(phoneStr, showMessage);
        }
    }
}
 ...
 */
function sendCartSms(_row, showMessage){
    var _obj = eval('('+_row+')');
...
function toSettleAccount() {
    ...
    if (!$("#leadId").val()) {
        var orderFlag=$("#createOrderRight").val();
        if(parentOrderId!=null&&orderFlag=="1"){
            //有创建子订单权限的人，不需要线索
            console.debug("can create order!");
        }
        else{
    ...
    $.get('/cart/validateShoppingCart/' + $("#shoppingcart_id").val()+'?parentOrderId='+parentOrderId,
        function (data) {
            if (data != null) {
                window.parent.alertWin('系统提示', data);
            } else {
...
function validateCreateOrder() {
    ...
    $.ajax({
        url:'/cart/validateShoppingCart/' + $("#shoppingcart_id").val()+'?parentOrderId='+parentOrderId,
        type: 'GET',
        dataType: 'JSON',
        async:false,
        success: function (data) {
    ...
    if($("#creditCardMobile").is(":visible")){
        if(!$("#id_creditCardMobile").val()){
            window.parent.alertWin('系统提示', "请录入信用卡绑定手机号码");
            return false;
        }
    }
    return true;
...
function  getdeliverMessage(paytype,audit){
    switch (paytype) {
        case "1":{
            if('true' == audit)        return"订单提交成功，等待主管审批通过后仓库发货，货到付款";
            else  return "订单提交成功，等待仓库发货，货到付款" ;
        }
        case "12":{
            if('true' == audit)       return "订单提交成功，等待主管审批通过及客户邮局汇款";
            else return "订单提交成功，等待客户邮局汇款";
        }
        case "13":{
            if('true' == audit)         return "订单提交成功，等待主管审批通过及客户银行转账";
            else    return "订单提交成功，等待客户银行转账";
        }
        case "11":{
            if('true' == audit)        return "订单提交成功，等待主管审批通过后客户上门自提";
            else        return "订单提交成功，等待客户上门自提";
        }
        /* case "2":{
...
function getCreditCartMobile(){
    var mobile =$("#id_creditCardMobile").val();
    if(mobile.indexOf("*") > -1){
        mobile = $("#id_creditCardMobile_hidden").val();
    }
    return mobile;
}
...
function cart_redirect(){
    if(potentialContactTabId){
        parent.gotoInfoCustomerOrderTab($("#customerId").val(),$("#customerType").val(),$("#name").val(), !window.parent.$("#"+window.parent.currentTabObj.tabId).hasClass("rell"));
        parent.destoryTab(potentialContactTabId);
    }else{
        window.location.href = '/contact/1/' + $("#shoppingcart_contactId").val() + "?selectedTab=1";
    }
}
...
function parentInfoSwitch(parentId)
{
    $("#parentOrderId").val(parentId);

    var htmlData=parentId;
    if(parentId!=null&&parentId!="")
    {
        htmlData='<a class="link" onclick="javascript:viewOrderDetail(' + parentId + ')">'+parentId+'</a>';
    }
    else
    {
        parentOrderId=null;
    }
    $("#parentInfo_id").html(htmlData);
    if(parentId!=null&&parentId!="")
    {
        $('#parentInfo').css({display:''});
    }
    else
    {
