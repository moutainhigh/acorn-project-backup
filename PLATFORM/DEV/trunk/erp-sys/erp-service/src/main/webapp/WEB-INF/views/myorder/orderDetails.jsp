...
<div id="loading_mask">
		...
		<div class="goods_info">
			<h3 class="goods_info_title">
				...
				<div id="id_order_op_div" class="fr">
                    <c:if test="${!editable and order.status!='0'}">
                    <a onclick="reiApplication()" class="history"><span></span>发票补发申请</a>

                    <a onclick="addChildOrder('${order.contactid}','${order.orderid}')" class="history"><span></span>添加子订单</a>
                    </c:if>
				...
				<h3 class="mt10">商品明细
					<c:if test="${editable && approvaled}">
						<a id="id_modify_product_a" class="buttonEdit" <c:if test="${!activable}">style="display: none"</c:if>
							href="${ctx}/contact/1/${order.contactid}?orderId=${order.orderid}&batchId=${batchId}&parentId=${order.parentid}" title="修改"></a>
					</c:if>
				</h3>
					...
					<c:forEach var="detail" items="${!empty editDetails ? editDetails : order.orderDetails}">
						...
						<tr>
                            ...
                            <c:forEach var="saleType" items="${saleTypes}">
                                <c:if test="${saleType.id.id == detail.soldwith}"><td>${saleType.dsc}</td></c:if>
                            </c:forEach>
                                <%--
    ...
    <div id="reiApplication" class="easyui-window" style="" data-options="title:'客户发票',closed:true,modal:true,draggable:false,top:100" >
    <div class="form-tabs">
        <div class="baseInfo" style="margin: 0">
            <table class="" border="0" cellspacing="0" cellpadding="0"
                ...
                <tr>
                    <td>订单号</td>
                    <td>        <input id="invoice_orderid" name="invoice_orderid" class="fl"  type="text" maxlength="20" disabled="true" value="${orderId}" />
                    </td>
                    <td >抬头</td>
                    <td><input id="invoice_title" name="invoice_title" class="fl"  type="text" maxlength="20" /></td>
                    <td>产品名称</td>
                    <td>
                        <input id="invoice_prodname" name="invoice_prodname" class="fl"  type="text" maxlength="20" disabled="true" />
                ...
                <tr>
                    <td>产品简码</td>
                    <td>  <input id="invoice_prodscode" name="invoice_prodscode" class="fl"  type="text" maxlength="20" disabled="true"/>
                    ...
                    <td>
                        <select id="invoice_freetype" name="invoice_freetype" class="easyui-combobox" required="true" style="width:134px" >
                            <option value="商品简码">商品简码</option>
                            <option value="商品名称">商品名称</option>
                        </select>
                    </td>
                    <td>联系电话</td>
                    <td>
                        <input id="invoice_phone" name="invoice_phone" type="text" maxlength="20" class="easyui-validatebox" data-options="required:true,validType:'phonevalidator'"/>
                    </td>
                </tr>
                <tr>
                    <td>订单总价</td>
                    <td>  <input id="invoice_ordprice" name="invoice_ordprice" class="fl"  type="text" maxlength="20" disabled="true" />
                    ...
                    <td>
                        <input id="invoice_mailprice" name="invoice_mailprice" class="fl"  type="text" maxlength="20" disabled="true"/>
                    </td>
                    <td>产品价格</td>
                    <td>
                        <input id="invoice_prodprice" name="invoice_prodprice" class="fl"  type="text" maxlength="20" disabled="true"/>
                    </td>
                </tr>
                <tr class="remarks">
                    <td valign="top">联系地址</td>
                    <td colspan="5"><textarea id="invoice_address" name="invoice_address" class="easyui-validatebox" required="true"></textarea>
                    </td>
                </tr>
                <tr class="remarks">
                    <td valign="top">备注</td>
                    <td colspan="5"><textarea id="invoice_comments" name="invoice_comments"></textarea>
                    <input id="invoice_mailtype" type="hidden"/>
                    </td>
                </tr>
            </table>
        </div>

        <p class="textC mt10"><a href="javascript:void(0);" id="invoice_save" onclick="" class="window_btn ml10">保存</a>
            <a href="javascript:void(0);" id="invoice_cancel" onclick="" class="window_btn ml10">取消</a></p>
    </div>
