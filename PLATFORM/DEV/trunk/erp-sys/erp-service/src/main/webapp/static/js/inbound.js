            ...
            {field:'addressEdit',title:'操作',align:'center',width:50,
                formatter:function(value, rowData, rowIndex){
                    ...
                    if ($("#customerId").val() == "") {
                        ...
                        if ($("#agentType").val().toLocaleLowerCase() == 'cpn' && rowData.addressid!=null) {
...
//            $('#dg').datagrid('fixDetailRowHeight',index);
        ...
        columns : [ [
            ...
            {field:'kfevent',title:'操作',width:100,align:'center',formatter:function(value,row){
                var _buttonWrap = $('<div></div>');
                var _eventButton = $('<a href="javascript:void(0);" title="事件" class="order_event mr10"></a>');
                _eventButton.attr('onclick','eventOrder('+row.orderid+')');
                _buttonWrap.append(_eventButton);
    ...
    if($("#page_refresh").val()){
        setTimeout(function(){
            console.debug("go to tab:"+$("#page_refresh").val());
...
function showShoppingCartPage(){
    if($("#shoppingcart_id").val()){
        ...
        if($("#order_id").val()) {
            ...
            {
                shoppingCartCreateUrl = shoppingCartCreateUrl+"/KF/"+$("#customerId").val();
            }
            else{
            shoppingCartCreateUrl = shoppingCartCreateUrl+"/KF/"+$("#customerId").val();
            }
...
function setParentOrderId(orderId)
{
    console.debug("parent order id:"+orderId);
    var tempOrderId = $("#parentOrderId").val();

    $("#parentOrderId").val(orderId);
    parentOrderId=orderId;
    parentInfoSwitch(parentOrderId);
    if(tempOrderId==null||tempOrderId=="")
    {
        switchContactOrderType(1);
    }
}

function clearParentOrderId()
{
    var tempOrderId = $("#parentOrderId").val();
    parentInfoSwitch("");
    if(tempOrderId!=null&&tempOrderId!="")
    {
        switchContactOrderType(0);
    }
}

function switchContactOrderType(type)
{
    //转换时，需要从新刷新购物车页面
    if(type==0)
    {
...
//跳转到事件管理
function cgotoEvent(customerId){
    var customerType =$("#customerType").val();
    var name=$("#name").val();
    if(customerType == 1){
    window.parent.addTab('E'+customerId,name+'-事件管理', false, "/case/index?module=inbound&contactId="+customerId,"",true);
    }else{
        window.parent.alertWin('系统提示', "潜客不支持该操作");
    }

}

function eventOrder(orderIdx)
{
    /*alert("订单号："+orderId);*/
    var contactId=$("#customerId").val();
    var contactname=$("#name").val();
    window.parent.addTab('E'+contactId, contactname+'事件管理', 'false','/case/index?contactId='+contactId+"&contactName="+encodeURIComponent(contactname)+"&orderId="+orderIdx);
}
