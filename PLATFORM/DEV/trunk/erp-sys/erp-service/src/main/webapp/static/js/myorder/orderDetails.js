...
function init() {
    ...
    $("#invoice_save").click(function(){
       //check
        var msg="保存发票申请失败";
        if($("#reiApplication").form('validate')){
            $.ajax({
                type: "post",
                async : false,
                url: "/invoice/add",
                data:{
                    orderid:$('#invoice_orderid').val(),
                    comments:$('#invoice_comments').val(),
                    title:$("#invoice_title").val(),
                    prodname:$("#invoice_prodname").val(),
                    prodscode:$("#invoice_prodscode").val(),
                    ordprice:$("#invoice_ordprice").val(),
                    mailprice:$("#invoice_mailprice").val(),
                    prodprice:$("#invoice_prodprice").val(),
                    address:$("#invoice_address").val(),
                    phone:$("#invoice_phone").val(),
                    freetype:$("#invoice_freetype").combobox('getValue'),
                    mailtype:$("#invoice_mailtype").val()
                },
                dataType:"json",
            ...
            window.parent.alertWin('系统提示', msg);
            $('#reiApplication').window('close');
        }
        else
        {
            console.debug("error---");
 ...
 * 发票补发申请
 */
function reiApplication(){
    $.ajax({
        type: "get",
        async : false,
        url: "/invoice/init?orderId="+$('#invoice_orderid').val(),
        success: function(data){
            $("#invoice_title").val(data.title);
            $("#invoice_prodname").val(data.prodname);
            $("#invoice_prodscode").val(data.prodscode);
            $("#invoice_ordprice").val(data.ordprice);
            $("#invoice_mailprice").val(data.mailprice);
            $("#invoice_prodprice").val(data.prodprice);
            $("#invoice_address").val(data.address);
            $("#invoice_phone").val('');
            $("#invoice_mailtype").val(data.mailtype);
        }
    });
    $('#reiApplication').window('open');
}
...
function initCreditTable() {
	$("#id_credit_table").datagrid({
		...
		columns: [[
			...
			{field: 'extraCode', title: '附加码', width: 50, formatter: enExtraCode},
...
function initCreditListTable(contactId) {
	$('#id_credit_list_table').datagrid({
        ...
        columns:[[
            ...
            {field: 'extraCode', title: '附加码', width: 60, formatter: enExtraCode},
...
function enExtraCode(value) {
	return value != null && value.length != 0 ? value.substr(0, 1) + "**" : "";
}
...
function editCredit(idx) {
	...
	if (category == "1") {
		...
		var opt = $("#input_cardnum").validatebox("options");
        opt.required = false;
        opt.validType = "";
		$("#input_cardnum").validatebox("validate");
	...
	_extra_code_val = enExtraCode(row.extraCode);
	$("#input_extracode").val(enExtraCode(row.extraCode));
    $("#input_extracode").attr("plaintext", row.extraCode);
	$("#input_extracode").bind("keyup", function() {
		if ($("#input_extracode").val() != _extra_code_val) {
			_extra_code_val = $("#input_extracode").val();
			var op = $("#input_extracode").validatebox("options");
			op.required = row.showextracode == '-1' ? true : false;
			op.validType = "validExtraCode";
...
function doSaveCredit() {
	...
	if (cetagory == "1") { // 只能修改身份信息
		...
		plaintext = $("#input_extracode").attr("plaintext");
		var extraCode = $("#input_extracode").val();
		if (plaintext == undefined) {
			row.extraCode = extraCode;
		} else {
			if (!hasMask(extraCode) && !isSame(plaintext, extraCode)) {
				row.extraCode = extraCode;
			} else {
				row.extraCode = plaintext;
			}
		}
		card.extraCode = row.extraCode;
...
function triggerCompare() {
	...
	if ($("#id_credit_table").length > 0 && $("#id_credit_table").datagrid("getRows").length > 0) {
		...
		isCreditChanged = !isSame(getOldValue("cardholder"), cardRow.contactName)
				...
				|| !isSame(getOldValue("extracode"), cardRow.extraCode);
		...
		if (!isSame(getOldValue("extracode"), cardRow.extraCode)) setHighlight("extracode");
		...
		setNewValue("extracode", isCreditChanged ? cardRow.extraCode : "&nbsp;");
	}
...
function addChildOrder(contactid,orderid)
{
    //跳转到客户tab页
    //检查是否已经存在，存在就直接赋值
    var contactFrame=window.parent.frames['p_'+contactid];
    if(contactFrame!=null)
    {
        //contactFrame.setParentOrderId(orderid);
        //window.parent.showTabAndCallFunc(contactid,null,null);
        window.parent.addTab2(contactid, "我的客户", false, '/contact/1/' + contactid+"?parentId="+orderid+'&selectedTab=4');
    }
    else
    {
        //不存在，那么直接传递值
        window.parent.addTab(contactid, "我的客户", false, '/contact/1/' + contactid+"?parentId="+orderid+'&selectedTab=4');
    }
