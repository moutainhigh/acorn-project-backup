    ...
    $('#myOrderlist_audit').datagrid({
        ...
        columns:[[
            ...
            {field:'right',title:'处理',width:80,editor:'text',align:'center',formatter: function(val, row,index){
                ...
                {
                    tableToken+=cancelEnableToken;
                    if(row.audittype != '新增订单' && row.audittype != '新增客户' && row.audittype != '取消订单' && currentUsrId==row.bpmCrtUsr) {
                ...
                else
                {
                    ...
                    if(val.indexOf("CONFIRM")!=-1 && (row.audittype != '新增订单' && row.audittype != '新增客户' && row.audittype != '取消订单' && currentUsrId==row.bpmCrtUsr))
                    {
                        tableToken+=confirmEnableToken;
                    }
                }
