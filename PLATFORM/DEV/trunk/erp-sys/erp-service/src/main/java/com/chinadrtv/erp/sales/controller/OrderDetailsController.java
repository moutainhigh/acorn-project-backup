...
public class OrderDetailsController {
	...
	public void init() {
        ...
        for (Names names : namesService.getAllNames("SALETYPE")) {
            soldwithMap.put(names.getId().getId(), names.getDsc());
        }
	}
	...
	public String viewOrder(@PathVariable("orderId") String orderId,
        ...
        model.addAttribute("saleTypes", JSONSerializer.toJSON(namesService.getAllNames("SALETYPE")));
	...
	public String checkMobileExists(@RequestBody Phone phone) {
		return String.valueOf(phoneService.checkExistPhone(phone));
	}
...
//        }
		...
		try {
			sysMessageService.addMessage(createMessage(SecurityHelper.getLoginUser().getUserId(),originalOrder.getOrderid()));
		} catch (Exception e) {
		}
