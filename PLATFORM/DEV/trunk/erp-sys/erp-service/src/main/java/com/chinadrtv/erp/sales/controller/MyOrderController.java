...
import com.chinadrtv.erp.service.core.service.KfOrderExtService;
...
public class MyOrderController {
    ...
    public ModelAndView myorder()  {
        ...
        modelAndView.addObject("startInvoiceDate",this.getInvoiceStartDate());
        ...
        boolean isSupervisor = AgentUserHelpler.isManager(agentUser);
        ...
        try
        {
            ...
            if(orderAuditExtDtoList!=null)
            {
                for(OrderAuditExtDto orderAuditExtDto:orderAuditExtDtoList)
                {
                    ...
                    if(orderAuditExtDto.getUserBpm()!=null)
                    {
                        myOrderFrontAuditDto.setBpmCrtUsr(orderAuditExtDto.getUserBpm().getCreateUser());
                    }
    ...
    public Map<String, Object> cancelOrder(HttpServletResponse response, String orderId,String remark)
    {
        ...
        boolean bSucc=false;
        String currentUsrId=this.getCurrentUsrId();
        try
        {
            response.reset();
            remark=URLDecoder.decode(remark,"UTF-8");
            orderhistService.saveOrderCancelRequestByUser(orderId, remark, currentUsrId);
            {
                map.put("succ","1");
                map.put("audit","1");
       ...
       try
       {
           if(bSucc==true)
           {
                ...
                sysMessage.setRecivierGroup(userService.getUserGroup(currentUsrId));
                sysMessage.setDepartmentId(userService.getDepartment(currentUsrId));
...
//		dto.setIsSuite();
    ...
    {
        Date dt=new Date();
        Date dt1= new Date(dt.getTime()-this.sourceConfigure.getInvoiceQueryDays()*86400000L);
        SimpleDateFormat simpleDateFormat=new SimpleDateFormat("yyyy-MM-dd");
        return simpleDateFormat.format(dt1);
    }
    ...
    {
        List<Names> namesList=namesService.getAllNames("SALETYPE");
        ...
        for(OrderExtDto orderExtDto:orderExtDtoList)
        {
            ...
            for(OrderDetail orderDetail:orderExtDto.getOrder().getOrderdets())
            {
                ...
                if(namesList!=null)
                {
                    for(Names names:namesList)
                    {
                        if(names.getId().getId().equals(contactOrderDetailFrontDto.getSoldwith()))
                        {
                            contactOrderDetailFrontDto.setSoldwith(names.getDsc());
                            break;
                        }
                    }
                }
