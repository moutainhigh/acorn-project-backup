...
public class ContactController extends BaseController {
    ...
    public ModelAndView get(@PathVariable("customerType") String customerType,
                            ...
                            @RequestParam(value = "parentId",required = false)String parentOrderId) {
        ...
        mav.addObject("parentOrderId", parentOrderId);
        mav.addObject("orderSaleTypes", JSONSerializer.toJSON(namesService.getAllNames("SALETYPE")));
        if(AgentUserHelpler.canCreateOrderNoLead(user))
            mav.addObject("createOrderRight", "1");
        else
            mav.addObject("createOrderRight", "0");
        ...
        try {
            ...
            if (orderId != null) {
                ...
                mav.addObject("selectedTab",4);
            } else {
                shoppingCart = shoppingCartService.findShoppingCartByContactId(Long.parseLong(customerFrontDto.getCustomerId()),ShoppingCart.KF);
                mav.addObject("allPoint",  (new BigDecimal(points)).setScale(0,RoundingMode.DOWN));
                mav.addObject("cart_cartType", ShoppingCart.KF);
            }
