...
import com.chinadrtv.erp.oms.controller.IgentResult;
...
public class AutoOperator {
	private static final Logger log = LoggerFactory.getLogger(AutoOperator.class);
    @Autowired
	...
	public void singleApproval(PreTrade preTrade){
	 ...
	 private String getPostUrl(PreTrade preTrade, String url) {
	    ...
	    private String postTrade(PreTrade preTrade, List<PreTradeDetail> list, String url, String result) {
	        try {

	            String r2 = "<ops_trade>" +
	                    "<ops_trade_id>" + preTrade.getTradeId() + "</ops_trade_id>" +
	                    "<tid>" + preTrade.getAlipayTradeId() + "</tid>" +
	                    "<retailer_id>ORD_" + preTrade.getTradeType() + "</retailer_id>" +
	                    "<customer_id>" + preTrade.getCustomerId() + "</customer_id>" +
	                    "<trade_from>" + preTrade.getTradeFrom() + "</trade_from>" +
	                    "<receiver_name>" + preTrade.getReceiverName() + "</receiver_name>" +
	                    "<receiver_mobile>" + preTrade.getReceiverMobile() + "</receiver_mobile>" +
	                    "<receiver_phone>" + preTrade.getReceiverPhone() + "</receiver_phone>" +
	                    "<receiver_province>" + preTrade.getReceiverProvince() + "</receiver_province>" +
	                    "<receiver_city>" + preTrade.getReceiverCity() + "</receiver_city>" +
	                    "<receiver_district>" + preTrade.getReceiverCounty() + "</receiver_district>" +
	                    "<receiver_street>" + preTrade.getReceiverArea() + "</receiver_street>" +
	                    "<receiver_address><![CDATA[" + preTrade.getReceiverAddress() + "]]></receiver_address>" +
	                    "<receiver_post_code>" + preTrade.getReceiverPostCode() + "</receiver_post_code>" +
	                    "<tms_code>" + preTrade.getTmsCode() + "</tms_code>" +
	                    "<payment>" + preTrade.getPayment() + "</payment>" +
	                    "<commission_fee>" + preTrade.getCreditFee() + "</commission_fee>" +
	                    "<retailer_commission_fee>" + preTrade.getRetailerCommissionFee() + "</retailer_commission_fee>" +
	                    "<platform_commission_fee>" + preTrade.getPlatformCommissionFee() + "</platform_commission_fee>" +
	                    "<credit_fee>" + preTrade.getCreditFee() + "</credit_fee>" +
	                    "<adv_fee>" + preTrade.getAdvFee() + "</adv_fee>" +
	                    "<jhs_fee>" + preTrade.getJhsFee() + "</jhs_fee>" +
	                    "<revenue>" + preTrade.getRevenue() + "</revenue>" +
	                    "<created>" + preTrade.getOutCrdt() + "</created>" +
	                    "<post_fee>" + preTrade.getShippingFee() + "</post_fee>" +
	                    "<invoice_comment> "
	            ...
	            for (PreTradeDetail pre : list) {
	                ...
	                r2 += "</sku_code><number>" + pre.getQty() + "</number><price>" + pre.getUpPrice() + "</price></sku>";
	            }
	            r2 += "</skus>" + "</ops_trade>";
	           log.info("xml:" + r2);
		...
		String json = gson.toJson(preTrade);
		 ...
		 losStr= igent_result ? "审核成功," + preTrade.getImpStatusRemark()
                ...
                log.error("pre trade Optimistic Locking error:"+preTrade.getTradeId()+"-"+preTrade.getVersion(),e);
				message = "重复订单";
			}catch (Exception e) {
				message = "系统异常";
		...
		private String postTradeJson(PreTrade preTrade,List<PreTradeDetail> list,String url,String crusr) {
			...
			try {
				...
				// TODO Auto-generated catch block
                log.error("pre trade add error1:"+preTrade.getTradeId()+"- dsc:"+e.getMessage(), e);
				//e.printStackTrace();
			} catch (IOException e) {
				// TODO Auto-generated catch block
                log.error("pre trade add error2:"+preTrade.getTradeId()+"- dsc:"+e.getMessage(), e);
				//e.printStackTrace();
			}
