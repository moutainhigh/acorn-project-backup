...
public class TaskController extends BaseController {
    ...
    @Value("${UNSTARTED_TASK_URL}")
	private String unstartTaskUrl;
    @Value("${AUDITTASK_URL}")
...
/*		isSupervisor |= user.hasRole(SecurityConstants.ROLE_OUTBOUND_GROUP_MANAGER);
	...
	@ResponseBody
	public Map<String, Object> queryCampaignTask(DataGridModel dataGrid, CampaignTaskDto campaignTaskDto) throws ServiceException{
    	Map<String, Object> pageMap = new HashMap<String, Object>();
		try {
	    	StringBuilder url = new StringBuilder(taskHost+taskUrl);
	
				url = buildParameters(url, dataGrid, campaignTaskDto);
			
			ResponseEntity<Map> response = null;
	    	try {
	    		response = restTemplate.getForEntity(url.toString(), Map.class);
			} catch (RestClientException e) {
				logger.error("get-task", e);
			}
	    	
	    	if(null ==response || response.getStatusCode().value() != 200) {
	    		throw new ServiceException("调用请求错误");
	    	}
	    	pageMap = response.getBody();
		} catch (Exception e) {
	...
	public Map<String, Object> queryUnstartedCampaignTask(DataGridModel dataGrid, CampaignTaskDto campaignTaskDto){
		Map<String, Object> pageMap = new HashMap<String, Object>();
		try {
	    	StringBuilder url = new StringBuilder(taskHost+unstartTaskUrl);
	...
	public Map<String, Object> queryAuditTask(DataGridModel dataGrid, ApprovingTaskDto approvingTaskDto){
		...
		try {
			if(isOutboundDepartmentManager || isInboundDepartmentManager || isOutboundGroupManager ||isInboundGroupManager) {
		    	StringBuilder url = new StringBuilder(taskHost+auditTaskUrl);
		    	url = buildParameters(url, dataGrid, approvingTaskDto);
				ResponseEntity<Map> response = null;
		    	try {
		    		response = restTemplate.getForEntity(url.toString(), Map.class);
				} catch (RestClientException e) {
	...
	public ModelAndView processAuditTask(
		...
		if(isOutboundDepartmentManager || isOutboundGroupManager
				|| isInboundDepartmentManager || isInboundGroupManager) {
			isManager = true;
		}
	...
	public void updateAuditTaskBatch(HttpServletResponse response,
		...
		if(auditInfos == null || auditInfos.length==0) {
            ...
            String strDebug="audit task begin:";
            for(AuditInfoDto auditInfoDto:auditInfos)
            {
                strDebug+=auditInfos[0].getId()+"-"+auditInfoDto.getBusiType();
                strDebug+=";";
            }
            logger.warn(strDebug);
			...
			try {
                ...
                logger.error("audit task marketing exp:"+auditInfos[0].getId(), e);
                ...
                logger.error("audit task erp exp:"+auditInfos[0].getId(), e);
...
//						tasks = userBpmTaskService.queryApprovingTaskByInstId(auditInfos[0].getInstId());
                ...
                logger.error("audit task unknown exp:"+auditInfos[0].getId(), e);
...
/***************************************取数***********************************************/
    ...
    private StringBuilder buildParameters(StringBuilder url, DataGridModel dataGrid, Object dto) throws IllegalArgumentException, IllegalAccessException {
		int paraCount = 0;
		Field[] modelFields = DataGridModel.class.getDeclaredFields();
		for(Field f: modelFields) {
			f.setAccessible(true);
			Object obj = f.get(dataGrid);
			...
			url.append(f.getName()+"="+obj);
			paraCount++;
		}
		Field[] fields = null;
		if(dto instanceof CampaignTaskDto) {
			fields = CampaignTaskDto.class.getDeclaredFields();
		} else if(dto instanceof ApprovingTaskDto){
			fields = ApprovingTaskDto.class.getDeclaredFields();
		}
		for(Field f: fields) {
			f.setAccessible(true);
