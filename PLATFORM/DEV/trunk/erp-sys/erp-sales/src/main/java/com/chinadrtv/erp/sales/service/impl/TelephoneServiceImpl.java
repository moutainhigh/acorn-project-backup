...
public class TelephoneServiceImpl implements TelephoneService {
	...
	*/ 
	...
	public CtiLoginDto queryCtiLoginInfo(String agentId,String ip) {
		...
		try {
			String deptNum = userService.getDepartment(agentId);
			if(!StringUtils.isEmpty(deptNum)){
				DepartmentInfo departInfo = userService.getDepartmentInfo(deptNum);
				result.setAreaCode(departInfo.getAreaCode());
			}
		} catch (Exception e) {
