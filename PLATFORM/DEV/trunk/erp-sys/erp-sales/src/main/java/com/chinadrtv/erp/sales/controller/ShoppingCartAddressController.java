...
public class ShoppingCartAddressController extends BaseController {
    ...
    public Map<String, String> getOrderType(@RequestParam(value = "instId", required = false) String instId,
                                            @RequestParam(value = "dnsi", required = false) String dnsi) {
    ...
    /*private String getLeadN400(String instId) throws ServiceException {
        if (StringUtils.isBlank(instId)) {
            return null;
        }
        try {
            CampaignTaskVO campaignTaskVO = campaignBPMTaskService.queryMarketingTask(instId);
            if (campaignTaskVO != null) {
                Lead lead = getLastLead(campaignTaskVO.getLeadId());
                if (lead != null && StringUtils.isNotBlank(lead.getDnis())) {
                    String n400 = orderhistService.queryN400ByDnis(lead.getDnis());
                    if (StringUtils.isNotBlank(n400)) {
                        return n400;
                    }
                }
            }
        } catch (Exception e) {
            logger.error("获取400号码失败", e);
        }
        return null;
    }
*/
    private Lead getLastLead(Long leadId) {
        Lead lead = leadService.get(leadId);
        if (lead.getPreviousLeadId() != null) {
            return getLastLead(lead.getPreviousLeadId());
        }
        return lead;
    }
