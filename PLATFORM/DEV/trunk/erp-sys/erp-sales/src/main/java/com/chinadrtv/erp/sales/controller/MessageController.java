...
public class MessageController {
    ...
    public  String updateMessageCheck(SysMessageDto sysMessageDto)
    {
        ...
        if(agentUser==null)
            return "";
        if(SecurityHelper.isManager(agentUser))
        {
    ...
    private List<String> convertMessageShow(List<SysMessage> sysMessageList)
    {
        ...
        int noReplayNotice = cusnoteService.queryCountNoRepalyNotice(SecurityHelper.getLoginUser().getUserId());
        if (noReplayNotice > 0) stringList.add("~有" + noReplayNotice + "条客服通知未回复");
