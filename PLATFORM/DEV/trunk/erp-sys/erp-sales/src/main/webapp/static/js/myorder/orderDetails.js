...
function binding() {
    ...
    $("#input_addressDesc_check").hide();
    var isInvalid = false;
    $("#input_addressDesc").bind("keyup", function() {
        isInvalid = validAddressDesc($("#input_addressDesc").val());
        if(isInvalid) {
            $("#input_addressDesc_check").show();
        } else {
            $("#input_addressDesc_check").hide();
        }
    });
...
function validAddressDesc(value) {
	var isInvalid = false;
	var inputValue = formatAddress(value);
	var keywords = $("#d_address_check", window.parent.document).val().split(",");
    var extKeywords = $("#d_address_check_ext", window.parent.document).val().split(",");
	// 包含
	for (var i = 0; i < keywords.length; i++)  {
		if(inputValue.indexOf(keywords[i]) >= 0) {
			isInvalid = true;
			break;
		} else {
			isInvalid = false;
	...
	// 不包含
	for (var i = 0; i < extKeywords.length; i++) {
		if(inputValue.indexOf(extKeywords[i]) >=0 ) {
			isInvalid = false;
			break;
		}
	}
