					...
					<table class="pro" width="404px" border="1" cellspacing="0" cellpadding="0">
						...
						<c:forEach var="detail" items="${order.orderDetails}">
							...
							<tr>
								...
								<td><div style="width: 53px">${detail.soldwith eq 1 ? '直接销售' : (detail.soldwith eq 2 ? '搭销' : '赠品')}</div></td>
							</tr>
						</c:forEach>
					</table>
				</td>
									...
									<c:choose>
										...
										<c:otherwise>
											...
											<tr>
												...
												<td><span ${style}>${detmap.key.soldwith eq 1 ? '直接销售' : (detmap.key.soldwith eq 2 ? '搭销' : '赠品')}</span></td>
											</tr>
										</c:otherwise>
									</c:choose>
								</c:forEach>
