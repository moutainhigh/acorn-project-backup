$(function(){
	...
	var isInvalidAddressExist = false;
...
/*            	if(row.instId != null && "" != row.instId) {
            ...
            {field:'address',title:'详细地址',width:10,resizable:false,
                ...
                styler: function(value,row,index){
                	...
                	var inputval = row.address;// formatAddress($("#address").val());
                    if(inputval==null)
                        inputval='';
                	for ( var i = 0; i < addressKey.length; i++) {
                		if (inputval.indexOf(addressKey[i]) >= 0) {
                			isInvalidAddress = true;
                			isInvalidAddressExist = true;
                			break;
                		} else {
...
/*                	if(row.instId != null && "" != row.instId) {
	...
	//	                        	 $("#newAddressTable_"+rowIndex).show();
		        	...
		        	if(isInvalidAddressExist){
		        		$("#div_contact_new_address_tip").html("红字为修改内容;&nbsp;黄底为无效地址");
		        		$("#div_contact_new_address_tip").show();
		        	}
	        	}
        	} else {
