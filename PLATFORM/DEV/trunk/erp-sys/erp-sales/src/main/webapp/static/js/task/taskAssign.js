...
$(function(){
	...
	$('#agentTb').datagrid({
		...
		onLoadSuccess : function() {
			...
			var _rows = $('#agentTb').datagrid('getRows');
			for(var t=0; t<_rows.length; t++) {
				var _agentId =  _rows[t].agentId;
				$.ajax({
					url: ctx + '/task/queryTaskCount',
					data: {'agentId': _agentId},
					type: 'GET',
					success: function(rs) {
						updateAgentTaskQty(rs);
					},error: function() {
						window.parent.alertWin('错误', '加载坐席[' + _row.agentId + ']任务数量失败');
						console.error('加载坐席[' + _row.agentId + ']任务数量失败');
					}
				});
			}
		},
		onLoadError : function() {
			console.error('加载坐席组信息失败');
			hideLoading();
			window.parent.alertWin('错误', '加载坐席组信息失败');
		},
		columns : [ [ 
		{
			field:'ck',
...
function queryGroup(){
	...
	if(null != groups && ''!=groups && $('#agentTb').length > 0) {
		showLoading();
		...
		$('#agentTb').datagrid('loadData', { total: 0, rows: [] });
	}
}


function updateAgentTaskQty(rs) {
	var _rows = $('#agentTb').datagrid('getRows');
	for(var i=0; i<_rows.length; i++) {
		var _agentId = _rows[i].agentId;
		if(_agentId == rs.agentId) {
			$('#agentTb').datagrid('updateRow',{
		        index: i,
		        row: {obtainQty: rs.obtainQty, obtainUnDialQty: rs.obtainUnDialQty}
		    });
		}
	}
}
...
function calcAverageCount(){
	...
	for(var i=0; i<groupRows.length; i++){
		...
		var _curGrpCount = 0;
		for(var m=0; m<agentRows.length; m++) {
			var _userWorkGrp = agentRows[m].userWorkGrp;
			if(_userWorkGrp == groupRow.grpid) {
				_curGrpCount ++;
			}
		}
		//每人每个
		var each = Math.floor(groupQty / _curGrpCount);
		//剩下的
		var left = Number(groupQty % _curGrpCount);
		...
		for(var j=0; j<agentRows.length; j++){
			...
			if(_gr.grpid == _group){
				...
				if(null!= _val && undefined != _val && ''!=_val){
					if(Number(agentRows[j].assignQty) > 100) {
						window.parent.alertWin('错误', '分配到坐席['+agentRows[j].agentId+']数量不能大于100，请少量多次分配！');
						return;
					}
...
function assignOperate(){
    ...
    for(var i=0; i<assignArr.length; i++) {
    	var _agent = assignArr[i];
    	obj['agentUsersStr'] = JSON.stringify(_agent);
		...
		$.ajaxQueue({
    		...
    		success : function(rs){
    			if(eval(!rs.success)){
    				//window.parent.alertWin('提示', '分配失败<br>' + rs.message);
    				errorArr.push(rs.message);
    				console.error('分配失败：' + rs.message);
    			}
    		},
    		error : function() {
    			hideLoading();
    			window.parent.alertWin('提示', '分配失败，网络错误');
    			return false;
    		}
    	});
    ...
    $.ajaxQueue().done(function(data) {
		...
		$('#workgroupT').datagrid('reload', {
			...
			onLoadSuccess: function() {
        		...
        		hideLoading();
        		var msg = '成功分配到[' + (assignArr.length - errorArr.length) + ']个用户，失败[' + errorArr.length + ']个用户<br>';
        		for(var i=0; i<errorArr.length; i++) {
        			msg += errorArr[i] + '<br>';
        		}
        		window.parent.alertWin('提示', msg);
			}
		});
