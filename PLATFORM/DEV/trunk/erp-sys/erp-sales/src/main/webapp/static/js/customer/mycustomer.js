	...
	if(!isINB) {
		...
		$('#AllCustomerDT').datagrid({
			...
			columns : [ [ 
			{
				...
				formatter: function(val, row){
					var _val = '';
					if(row.customerOwner==1 || row.customerOwner==3 || taskPrivilege){
        ...
        columns : [ [
            {
                ...
                formatter: function(val, row){
                    var url;
                    if(row.status=='已拒绝' && row.busiType != '新增客户' && row.busiType != '新增订单' && curUid==row.crUser) {
