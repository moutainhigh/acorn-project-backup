...
$(function(){
    ...
    if(isQualityInspector==true)
    {
        if($('#tbCrusr').attr('disabled')!=false)
        {
            console.debug($('#tbCrusr').attr('disabled'));
            $('#tbCrusr').removeAttr('disabled');
        }
    }
...
/*    $('.easyui-datebox').datebox({
    ...
    $.extend($.fn.validatebox.defaults.rules, {
    	validateAccount: {
    		validator: function(value, param) {
    			var _val = $("#d_seat").combobox('getValue');
    			if(_val.length<4) {
    				return false;
    			}
                else
                {
                    var valid=false;
                    var datas=$("#d_seat").combobox('getData');
                    if(datas!=null&&datas.length>0)
                    {
                        for(var i=0;i<datas.length;i++)
                        {
                            if(datas[i].userId==_val)
                            {
                                valid=true;
                                break;
                            }
    ...
    $("#d_seat").combobox({
        ...
        keyHandler: {
            query: function(q){
            	if(q.length>=4){
                    $.get('/common/getUserByUid?uid='+q, function(result){
                        $("#d_seat").combobox('loadData',result);
                        $("#d_seat").combobox('setValue',q);
                    });
                }
                else
                {
                    $("#d_seat").combobox('loadData',null);
                    $("#d_seat").combobox('setValue',q);
                }
            }
        },
...
//                return tableToken;
    ...
    $('#myOrdersearch').click(function(){
        if(!$("#myOrderForm").form("validate"))
        {
            console.debug("invalidate data!!!");
            return;
        }
        /*if(isManager==true||isQualityInspector==true)
        {
        ...
        if(isManager) {
        	...
        	//订购和分拣状态的订单
        	if(('1' == _orderStatus || '2' == _orderStatus || '7' == _orderStatus) && null != _agentId) {
        		$('#myOrderallot').show();
...
//    });
    ...
    $('#myOrderlist_pay').datagrid({
        ...
        columns:[[
            ...
            {field:'right',title:'操作',align:'center',width:80,editor:'text',formatter: function(val, row,index){
                ...
                var claimEnableToken= '<a href="javascript:void(0);" onclick="javascript:claimOrder('+'\'myOrderlist_pay\','+index+','+row.orderid+',\''+row.bankName+'\')" class="cr_card mr10" title="索权"></a>';
    ...
    $('#myOrdersearch_pay').click(function(){
        if(!$("#myOrderForm_pay").form("validate"))
        {
            console.debug("invalidate pay data!!!");
            return;
        }
       /* if(isManager==true)
        {
    ...
    $('#myOrderlist_audit').datagrid({
        ...
        columns:[[
            ...
            {field:'right',title:'处理',width:80,editor:'text',align:'center',formatter: function(val, row,index){
                ...
                {
                    tableToken+=cancelEnableToken;
                    if(row.audittype != '新增订单' && row.audittype != '新增客户' && row.audittype != '取消订单' && currentUsrId==row.crusr) {
                ...
                else
                {
                    ...
                    if(val.indexOf("CONFIRM")!=-1 && (row.audittype != '新增订单' && row.audittype != '新增客户' && row.audittype != '取消订单' && currentUsrId==row.crusr))
                    {
                        tableToken+=confirmEnableToken;
                    }
                }
    ...
    $('#myOrdersearch_audit').click(function(){
        if(!$("#myOrderForm_audit").form("validate"))
        {
            console.debug("invalidate audit data!!!");
            return;
        }
...
//    });
    ...
    $('#orderClaim_confirm').click(function(){
        if(!$("#fmClaim").form("validate"))
        {
            return;
        }

        var orderId=$("#txtClaimOrderId").val();
        var phone = $("#txtClaimPhone").combobox('getValue');
        if (phone==null||phone=='') {
            phone =  $("#txtClaimPhone").combobox('getText');
        }

        var credentialsNo=$("#txtClaimCredentials").combobox('getValue');

        $.ajax({
            url:"/cardpay/grant",
            async : false,
            type: "post",
            data:{orderId:orderId,cvv:$("#txtCardCVV").val(),phone:phone,credentialsNo:credentialsNo},
            success:function(data){
                if(data!=null)
                {
                    window.parent.alertWin('系统提示', data.msg);
    ...
    $("#txtClaimCredentials").combobox({
        url : '/cardpay/getCredentials',
        valueField : 'id',
        textField : 'value',
        editable:false,
    ...
    $("#txtClaimPhone").combobox({
        url : '/cardpay/getCardpayPhones',
        valueField : 'phn2',
        textField : 'phoneMask',
        onBeforeLoad : function(param) {
            return false;
        ...
        onLoadSuccess:function(){
            var datas=$("#txtClaimPhone").combobox('getData');
            if(datas!=null&&datas.length!=null&&datas.length>0)
            {
                $("#txtClaimPhone").combobox('select',datas[0].phn2);
            }
            else
            {
            }
...
function order_cardpay_reset()
{
    $('#txtCardCVV').val('');
    $('#txtClaimPhone').combobox('clear');
    $('#txtClaimCredentials').combobox('clear');
}

function order_cardpay_show(orderId,bCVV, bPhone, bCredentials)
{
    if(bCVV==true)
    {
        $('#txtCardCVVLabel').css({display:''});
        $('#txtCardCVV').css({display:''});
        $("#txtCardCVV").validatebox({required: true});
    }
    else
    {
        $('#txtCardCVVLabel').css({display:'none'});
        $('#txtCardCVV').css({display:'none'});
        $("#txtCardCVV").validatebox({required: false});
    }
    if(bCredentials==true)
    {
        $('#txtClaimCredentialsLabel').css({display:''});
        $('#dvClaimCredentials').css({display:''});
        $("#txtClaimCredentials").combobox({required: true});
        //获取证件数据
        var url = '/cardpay/getCredentials';
        $.ajax({
            url:url,
            type: "post",
            data:{orderId:orderId},
            success:function(data){
                if(data!=null)
                {
                    $('#txtClaimCredentials').combobox('loadData',data);
                }
            }
        });
    }
    ...
    {
        $('#txtClaimCredentialsLabel').css({display:'none'});
        $('#dvClaimCredentials').css({display:'none'});
        $("#txtClaimCredentials").combobox({required: false});
    }

    if(bPhone==true)
    {
        $('#txtClaimPhoneLabel').css({display:''});
        $('#dvClaimPhone').css({display:''});
        $("#txtClaimPhone").combobox({required: true});
        var url = '/cardpay/getCardpayPhones';
        $.ajax({
            url:url,
            type: "post",
            data:{orderId:orderId},
            success:function(data){
                if(data!=null)
                {
                    $('#txtClaimPhone').combobox('loadData',data);
                }
            }
        });
    }
    ...
    {
        $('#txtClaimPhoneLabel').css({display:'none'});
        $('#dvClaimPhone').css({display:'none'});
        $("#txtClaimPhone").combobox({required: false});
    }
}

function claimOrder(listName, index, orderId,bankName)
{
    //check
    var pass=true;
    var stage=0;
    ...
    $.ajax({
        url:"/cardpay/grantCheck/"+orderId,
        async : false,
        success:function(data){
            if(data!=null)
            {
                ...
                {
                    stage=data.stage;
                    if(bankName==icbcCardName)
                        $("#txtCardCVV").val(data.cvv);
                }
            }
        }
    });
    ...
    {
        if(stage==1)
            order_cardpay_show(orderId,true,true,true);
        else
            order_cardpay_show(orderId,true,false,true);
    }
    else
    {
        order_cardpay_show(orderId,false,false,false);
    }
        ...
        buttons: [{
            ...
            handler: function() {
                ...
                $.ajax({
                    url:"/myorder/myorder/cancel_audit",
                    contentType:"application/json; charset=utf-8",
                    data: {"batchId":batchId,"orderId":orderId,"remark":encodeURI(remark)},
                    success:function(data){
                        if(data!=null)
                        {
                            var showInfo=data.substr(1);
...
function isClaimEnable(cardtype)
{
    ...
    strIndex=cardtype.indexOf(icbcCardName);
    if(strIndex>=0)
    {
        return true;
    }
	...
	$.ajax({
		...
		success : function(rs) {
			if(eval(rs.success)) {
				...
				window.parent.alertWin('提示', '分配跟单任务失败:<br>' + rs.message);
